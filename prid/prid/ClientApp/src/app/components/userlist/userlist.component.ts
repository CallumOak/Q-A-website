...
export class UserListComponent implements AfterViewInit, OnDestroy {
    displayedColumns: string[] = ['pseudo', 'email', 'lastName', 'firstName', 'birthDate', 'roleAsString', 'reputationAsString', 'actions'];
    ...
    ngAfterViewInit(): void {
        ...
        this.dataSource.filterPredicate = (data: User, filter: string) => {
            const str = data.pseudo + ' ' + data.lastName + ' ' + data.firstName + ' ' + data.birthDate + ' ' + data.roleAsString;
    ...
    edit(user: User) {
        ...
        dlg.beforeClose().subscribe(res => {
            if (res) {
                _.assign(user, res);
                this.userService.update(res).subscribe(answer => {
                    if (!answer) {
                        this.snackBar.open(`There was an error at the server. The update has not been done! Please try again.`, 'Dismiss', { duration: 10000 });
                        this.refresh();
                    } else if (this.authService.currentUser.id == user.id)
                    {
